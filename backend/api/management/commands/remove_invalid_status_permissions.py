"""
Management command to remove permissions where component is not 'contacts' but has statusId.
Only 'contacts' component should have status-specific permissions.
"""
...
class Command(BaseCommand):
    help = 'Remove permissions where component is not contacts but has statusId'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--keep-contacts',
            action='store_true',
            help='Keep contacts component permissions even if they have statusId (default: only contacts can have statusId)',
        )

    def handle(self, *args, **options):
        dry_run = options['dry_run']
        keep_contacts = options['keep_contacts']
        
        # Find permissions where component is not 'contacts' but has statusId
        # Only 'contacts' component should have status-specific permissions
        # 'statuses' component should also NOT have statusId (we removed those earlier)
        if keep_contacts:
            # Remove all non-contacts components with statusId (including statuses)
            invalid_permissions = Permission.objects.filter(
                status__isnull=False
            ).exclude(component='contacts')
        else:
            # Only keep contacts component with statusId, remove everything else (including statuses)
            invalid_permissions = Permission.objects.filter(
                status__isnull=False
            ).exclude(component='contacts')
        ...
        if count == 0:
            self.stdout.write(
                self.style.SUCCESS(
                    'No invalid permissions found. All permissions with statusId are for "contacts" component.'
                )
            )
            return
        ...
        self.stdout.write(
            self.style.WARNING(
                f'Found {count} permission(s) where component is not "contacts" but has statusId:'
            )
        )
        ...
        if dry_run:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    ...
                    'where component is not "contacts" but has statusId.'
                )
            )
